/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   signal.c                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: bruno <bruno@student.42.fr>                +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/07/22 03:18:24 by bruno             #+#    #+#             */
/*   Updated: 2024/11/05 18:40:26 by bruno            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/minishell.h"

void	ignore_signal(struct sigaction *sa, int signal)//learn what it does
{
	struct sigaction	original_sa;
	int					original_flags;

	original_flags = sa->sa_flags;
	sa->sa_handler = SIG_IGN;
	sa->sa_flags |= SA_SIGINFO;
	if (sigemptyset(&sa->sa_mask) != 0)
		return ;
	sigaction(signal, sa, &original_sa);
	sa->sa_flags = original_flags;
}

void	root_handler(int signal, siginfo_t *info, void *context)
{
	(void)info;
	(void)context;
	if (signal == SIGINT)
	{
		ft_printf("\n");
		rl_on_new_line();
		rl_replace_line("", 0);
		rl_redisplay();
	}
}

void	here_handler(int signal, siginfo_t *info, void *context)
{
	(void)info;
	(void)context;
	if (signal == SIGINT)
	{
		ft_printf("\n");
		exit (130);
	}
}

/* void	signal_aux(t_signal type, struct sigaction sa)
{
	if (type == HEREDOC_SIG)
	{
		sa.sa_sigaction = here_handler;
		sa.sa_flags = SA_SIGINFO;
		sigemptyset(&sa.sa_mask);
		sigaction(SIGINT, &sa, NULL);
		ignore_signal(&sa, SIGQUIT);
	}
	else if (type == IGNORE_SIG)
	{
		ignore_signal(&sa, SIGINT);
		ignore_signal(&sa, SIGQUIT);
	}
} */
void	choose_sig(t_signal type)
{
	static struct sigaction	sa;

	if (type == ROOT_SIG)
	{
		sa.sa_sigaction = root_handler;//why different 2
		sa.sa_flags = SA_SIGINFO;//why different 1
		sigemptyset(&sa.sa_mask);
		sigaction(SIGINT, &sa, NULL);
		ignore_signal(&sa, SIGQUIT);
	}
	else if (type == CHILD_SIG)
	{
		sa.sa_handler = SIG_DFL;//why change handler to this
		sa.sa_flags = 0;//why different 1
		sigemptyset(&sa.sa_mask);
		sigaction(SIGINT, &sa, NULL);
		sigaction(SIGQUIT, &sa, NULL);
	}
	else if (type == HEREDOC_SIG)
	{
		sa.sa_sigaction = here_handler;//why different 2
		sa.sa_flags = SA_SIGINFO;//why different 1
		sigemptyset(&sa.sa_mask);
		sigaction(SIGINT, &sa, NULL);
		ignore_signal(&sa, SIGQUIT);
	}
	else if (type == IGNORE_SIG)
	{
		ignore_signal(&sa, SIGINT);//what is this for
		ignore_signal(&sa, SIGQUIT);//what is this for
	}
/* 	else
		signal_aux(type, sa); */
}

void	ctrld(char *line, t_env *env)
{
	free (line);
	printf("exit\n");
	if (env->env)
		free_array(env->env);
	rl_clear_history();
	exit (env->status);
}
