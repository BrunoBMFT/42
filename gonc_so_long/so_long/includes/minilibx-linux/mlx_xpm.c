...
void	*mlx_int_varsse_xpm(t_xvar *xvar,void *info,int info_size,char *(*f)())
{
		int		pos;
		char	*line;
		char	**tab;
...
void	*mlx_xpm_file_to_image(t_xvar *xvar,char *file,int *width,int *height)
{
		...
		if (img = mlx_int_varsse_xpm(xvar,ptr,size,mlx_int_get_line))
		{
				*width = img->width;
				*height = img->height;
		}
...
void	*mlx_xpm_to_image(t_xvar *xvar,char **xpm_data,int *width,int *height)
{
		...
		if (img = mlx_int_varsse_xpm(xvar,xpm_data,0,mlx_int_static_line))
		{
				*width = img->width;
				*height = img->height;
		}
