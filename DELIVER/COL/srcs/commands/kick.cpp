...
void	Server::commandKick(int i, std::string args)
{
	...
	//TODO choose if we find it by # or space
	//std::cout << "NICK KICK BEFORE:|" << toKickName << "|" << std::endl;
	/**
	 * Change by adding just a conditional to see if theres a channel (#) in the kick name
	 * and if its there it will just take it out
	 */
	if (toKickName.find('#') != std::string::npos)
		toKickName = toKickName.substr(toKickName.find(' ') + 2, toKickName.length() - toKickName.find(' ') - 2);
	//std::cout << "NICK KICK AFTER:|" << toKickName << "|" << std::endl;
