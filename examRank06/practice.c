...
int main(int ac, char **av) {
	...
	while (1) {
		...
		if (FD_ISSET(sockfd, &read_set)) {
			clfd = accept(sockfd, (struct sockaddr *)&cli, &len);
			if (clfd >= 0) {
				cls[clfd].id = id++;
				cls[clfd].m = NULL;
				FD_SET(clfd, &active_set);
				if (clfd > maxfd) maxfd = clfd;
				
				char m[MSIZE];
				bzero(m, MSIZE);
				sprintf(m, "server: client %d just arrived\n", cls[clfd].id);
				broadcast(clfd, maxfd, write_set, m);
			}
		}
		else {
			for (clfd = 0; clfd <= maxfd; clfd++) {
				printf("here\n");
				if (FD_ISSET(clfd, &read_set)) {
					char buf[BSIZE];
					bzero(buf, BSIZE);
					if (recv(clfd, buf, BSIZE - 1, 0) <= 0) {
						char m[MSIZE];
						bzero(m, MSIZE);
						sprintf(m, "server: client %d just left\n", cls[clfd].id);
						broadcast(clfd, maxfd, write_set, m);
						FD_CLR(clfd, &active_set);
						close(clfd);
						if (cls[clfd].m) {
							free (cls[clfd].m);
							cls[clfd].m = NULL;
						...
						char *line = NULL;
						cls[clfd].m = strjoin(cls[clfd].m, buf);
						while (extract(&(cls[clfd].m), &line)) {
							char m[MSIZE + strlen(line)];
							bzero(m, MSIZE + strlen(line));
							sprintf(m, "client %d : %s", cls[clfd].id, line);
							broadcast(clfd, maxfd, write_set, m);
							free(line);
							line = NULL;
						}
